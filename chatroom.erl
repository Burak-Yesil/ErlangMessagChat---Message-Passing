...
do_propegate_message(State, Ref, ClientPID, Message) ->
	...
	OtherClients = maps:keys(maps:filter(fun(K,_) -> K /= ClientPID end, State#chat_st.registrations)),
	CliNick = maps:get(ClientPID, State#chat_st.registrations),
	NewHistory = State#chat_st.history ++ [{CliNick, Message}],
	lists:foreach(fun(Client) -> Client!{request, self(), Ref, {incoming_msg, CliNick, State#chat_st.name, Message}} end, OtherClients),
	#chat_st{
		name = State#chat_st.name,
